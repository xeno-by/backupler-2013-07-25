...
trait FastTrack {
  ...
  private implicit def context2quasiquotes(c0: MacroContext): QuasiquoteImpls { val c: c0.type } =
  ...
  lazy val fastTrack = Map[Symbol, FastTrackEntry](
    ...
    make(       QuasiquoteClass_q_apply) { case _                                               => _.applyQ },
    make(     QuasiquoteClass_q_unapply) { case _                                               => _.unapplyQ },
    make(      QuasiquoteClass_tq_apply) { case _                                               => _.applyTq },
    make(    QuasiquoteClass_tq_unapply) { case _                                               => _.unapplyTq }
  )
}
