...
trait Macros extends FastTrack with MacroRuntimes with Traces with Helpers {
  ...
  private abstract class TypeMacroExpander[Result: ClassTag](role: MacroRole, typer: Typer, original: Tree, val tpt: Tree, val targs: List[Tree], val argss: List[List[Tree]])
    ...
    override def onDelayed(expanded: Tree): Result = {
      // TODO: we should probably call `typer.instantiate` here, but the problem is that
      // it doesn't immediately work, because the culprit(s) of the delay (uninferred type parameters)
      // are somehow not registered in the context, hence `context.extractUndetparams` does nothing
      // after fighting with it for a while, I'm just moving `run/macro-expand-tparams-implicit-typemacros-e` into pending for the time being
  ...
  def macroExpandAppliedType(typer: Typer, expandee: Tree, mode: Mode) = {
    ...
    object expander extends TypeMacroExpander[Tree](APPLIED_TYPE_ROLE, typer, expandee, tpt, targs, argss) {
      override def isTypeLike = true
      override def prepare(desugared: Tree) = typer.typed(desugared, EXPRmode, WildcardType)
      override def onSuccess(expanded: Tree) = typer.typed1(expanded, mode | Mode.FUNmode | Mode.TAPPmode, WildcardType)
      override def allowResult(result: Tree) = allowResultTree(result)
    }
    expander(expandee)
  }
